...
st.set_page_config(page_title="ISA 실시간 감시 (지수 우회형)", layout="wide")
...
# 3. [우회] 네이버 금융 실시간 지수 조회 (데이터 타입 에러 방어)
def get_market_index_safe():
    try:
        ...
        while len(results) < 2: results.append((0.0, 0.0))
...
if token:
    ...
    with c3:
        st.write(f"⏱️ **마지막 갱신:** {datetime.now(KST).strftime('%H:%M:%S')}")
        if st.button("🔄 실시간 시세 동기화"): st.rerun()
...
def load_data(token):
    try:
        ...
        prog = st.progress(0, text="한투 서버 연결 중...")
...
if not res_df.empty:
    ...
    def style(s):
        ...
        s.applymap(lambda v: 'background-color: #ff4b4b; color: white;' if "🚨" in str(v) else 'background-color: #ffa421; color: black;' if "⚠️" in str(v) else 'background-color: #28a745; color: white;' if "✅" in str(v) else '', subset=['상태'])
        s.set_properties(subset=['현재가'], **{'color': '#00d1ff', 'font-weight': 'bold'})
        return s
    st.dataframe(style(view.style.format({'현재가': '{:,.0f}', '등락률': '{:+.2%}', '기준고점': '{:,.0f}', '손절(-10%)': '{:,.0f}', '손절(-15%)': '{:,.0f}'})), use_container_width=True, height=600)
