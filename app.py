...
def get_naver_index():
    """네이버 실시간 JSON 구조(closePrice, fluctuationsRatio) 맞춤 파싱"""
    try:
...
if token:
    ...
    try:
        ...
        status_list = []
        prog_bar = st.progress(0, text="📊 실시간 시세 분석 중...")
        ...
        for i, row in df.iterrows():
            ...
            prog_bar.progress((i+1)/len(df), text=f"분석 중: {row['종목명']}")
            time.sleep(0.1)
